...
app.get("/", async function(req, res) {
    ...
    if (req.get("host").includes("hostlockers")) {
        ...
        content.title = entreprise;
    } else if (req.get("host").includes("foodlockers")) {
        entreprise = "foodlockers";
        content.title = entreprise;
    } else if (req.get("host").includes("servicelockers")) {
        entreprise = "servicelockers";
        content.title = entreprise;
    } else if (req.get("host").includes("sportlockers")) {
        entreprise = "sportlockers";
        content.title = entreprise;
    } else if (req.get("host").includes("energylockers")) {
        entreprise = "energylockers";
        content.title = entreprise;
    } else {
    ...
    const siteResult = await axios.get(
        "https://api.dev.smartlockers.io/api/v2.0/entreprise/bySite?filter=webSite&value=www.smartlockers.io"
    );
    content.site = siteResult.data.data;
